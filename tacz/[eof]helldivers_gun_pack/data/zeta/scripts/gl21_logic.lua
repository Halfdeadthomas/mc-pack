...
local function special_reload(api)
    local max_ammo = api:getMaxAmmoCount()
    local amount = api:consumeAmmoFromPlayer(5)
    local unit = math.floor(max_ammo/5)
    local ammo = unit * amount
    if amount > 0 then
        api:removeAmmoFromMagazine(api:getAmmoAmount())
        api:putAmmoInMagazine(ammo)
    end
end
