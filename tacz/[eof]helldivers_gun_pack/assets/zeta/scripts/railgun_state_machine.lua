...
function bolt_caught_states.normal.update(this, context)
    ...
    if ((this.bolt_caught_states.trigger)and(t2 - t1 > 2*itv+edge)) then
        ...
        if this.bolt_caught_states.proc>=9 then
    ...
    if ((this.bolt_caught_states.proc) / max) < (1/6) then
        process_display = 4 * (this.bolt_caught_states.proc/ max)
    else
        process_display = 2/3 + math.pow((this.bolt_caught_states.proc/ max - (1/6)), 0.5)*(2/5)
    end
    ...
    local track = context:findIdleTrack(GUN_KICK_TRACK_LINE, false)
        ...
        if (input == INPUT_SHOOT) then
            ...
            if (t2 - t1 <= itv+edge) then
                local mark = this.bolt_caught_states.proc
                if (mark) <=0 then
                    context:runAnimation("charge_start", track, true, PLAY_ONCE_STOP, 0)--charge_start里只放开始蓄力的音效
                end
                -- 没有松手（宽限为edge/ms）
                if this.bolt_caught_states.proc > max then
                    ...
                    if context:getFireMode() == BURST then
                        ...
                        this.bolt_caught_states.proc = math.min(10,this.bolt_caught_states.proc + step)
                    end
