...
local CHARGE_SOUND_TRACK = increment(static_track_top)
...
function bolt_caught_states.normal.update(this, context)
    ...
    if ((this.bolt_caught_states.trigger)and(t2 - t1 > 2*itv+edge))and(t1~=-1) then
        local track = context:findIdleTrack(GUN_KICK_TRACK_LINE, false)
        if this.bolt_caught_states.proc>=24 then
            context:runAnimation("shoot", track, true, PLAY_ONCE_STOP, 0)
            if this.bolt_caught_states.proc>=55 then
                 context:runAnimation("shoot_perfect", track, true, PLAY_ONCE_STOP, 0)--时机正确时播放的shoot动画，会覆盖正常shoot，不做这个动画就不覆盖
            end
        else
            context:runAnimation("charge_error", track, true, PLAY_ONCE_STOP, 0)--低蓄发射失败音频
        end
        context:pauseAnimation(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK))
        context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK),0.001,false)
        context:stopAnimation(context:getTrack(STATIC_TRACK_LINE,CHARGE_SOUND_TRACK))
        this.bolt_caught_states.trigger = false
        this.bolt_caught_states.proc = 0
    end
    if this.bolt_caught_states.trigger then
        context:resumeAnimation(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK))
    end
end
...
function bolt_caught_states.normal.entry(this, context)
    ...
    context:runAnimation("charge",context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), false, PLAY_ONCE_STOP, 0)
    context:pauseAnimation(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK))
end
...
function gun_kick_state.transition(this, context, input)
    local track = context:findIdleTrack(GUN_KICK_TRACK_LINE, false)
        ...
        if (input == INPUT_SHOOT) then
            ...
            local t2 = context:getCurrentTimestamp()
            local itv = context:getShootInterval()
            if this.gun_kick_state.sound_lock then
                context:runAnimation("charge_start", context:getTrack(STATIC_TRACK_LINE,CHARGE_SOUND_TRACK), false, PLAY_ONCE_STOP, 0)
            end
            if (t2 - t1 <= itv+edge) then
                this.gun_kick_state.sound_lock = false
                if this.bolt_caught_states.proc>0 then
                    context:runAnimation("charge_spin", track, true, PLAY_ONCE_HOLD, 0)
                end
                -- 没有松手（宽限为edge/ms）
                if this.bolt_caught_states.proc >= max then
                    this.bolt_caught_states.proc = 0
                    this.gun_kick_state.sound_lock = true
                    ...
                    this.bolt_caught_states.proc = this.bolt_caught_states.proc + step
                end
                this.bolt_caught_states.trigger = true
            else
                this.gun_kick_state.sound_lock = true
                this.bolt_caught_states.proc = 0
                this.bolt_caught_states.trigger = false
...
local M = {
    ...
    CHARGE_SOUND_TRACK = CHARGE_SOUND_TRACK,
    -- 状态
...
function M:initialize(context)
    ...
    self.bolt_caught_states.proc = 0
    self.bolt_caught_states.trigger = false
    self.gun_kick_state.sound_lock = false
end
