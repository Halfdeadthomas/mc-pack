...
local SOUND_TRACK = increment(static_track_top)
local HEAT_TRACK = increment(static_track_top)
...
local function getRandom(flag, context)
    local time = (context:getCurrentTimestamp())
    math.randomseed(tonumber(tostring(time):reverse():sub(1, 6)))
    local num = math.random(flag)
    return num
end

local function playSound(track, context)
    local flag = getRandom(11, context)
    local animationName = "sound_" .. tostring(flag)
...
function sound_state.static.update(this, context)
    ...
    if (nowTime - lastShoot > shootInterval + 100) then
        sound_state.shoot_time = 0
    end
    if (sound_state.shoot_time > sound_state.sound_time) then
        context:trigger(this.INPUT_PLAY_SOUND)
        sound_state.shoot_time = 0
    end
end

function sound_state.static.transition(this, context, input)
    if (input == this.INPUT_PLAY_SOUND) then
        return sound_state.playing
    end
end

function sound_state.playing.entry(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, SOUND_TRACK)
    playSound(track, context)
end

function sound_state.playing.update(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, SOUND_TRACK)
    if (context:isStopped(track)) then
        context:trigger(this.INPUT_STOP_SOUND)
    end
end

function sound_state.playing.transition(this, context, input)
    if (input == this.INPUT_STOP_SOUND) then
        sound_state.shoot_time = 0
        return sound_state.static
    end
end
...
function base_track_state.entry(this, context)
    ...
    context:runAnimation("heat", context:getTrack(STATIC_TRACK_LINE, HEAT_TRACK), false, LOOP, 0)
    context:pauseAnimation(context:getTrack(STATIC_TRACK_LINE, HEAT_TRACK))
end

function base_track_state.update(this, context)
    local heat = context:getHeatProgress()
    context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, HEAT_TRACK),heat,true)
end
...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        ...
        sound_state.shoot_time = sound_state.shoot_time + shootInterval
    end
    return nil
end
...
local M = {
    ...
    INPUT_STOP_SOUND = "stop_sound",
    INPUT_PLAY_SOUND = "play_sound"
}
