...
local RANDOM_X1 = increment(static_track_top)
local RANDOM_Y1 = increment(static_track_top)
local RANDOM_X2 = increment(static_track_top)
local RANDOM_Y2 = increment(static_track_top)
local RANDOM_X3 = increment(static_track_top)
local RANDOM_Y3 = increment(static_track_top)
local RANDOM_X4 = increment(static_track_top)
local RANDOM_Y4 = increment(static_track_top)
local RANDOM_X5 = increment(static_track_top)
local RANDOM_Y5 = increment(static_track_top)
local RANDOM_X6 = increment(static_track_top)
local RANDOM_Y6 = increment(static_track_top)
...
local base_track_state = {
    random_x = {0, 0, 0, 0, 0, 0},
    random_y = {0, 0, 0, 0, 0, 0},
}

local function runAndPauseAnimation(context, animName, trackKey)
    local track = context:getTrack(STATIC_TRACK_LINE, trackKey)
    context:runAnimation(animName, track, true, LOOP, 0)
    context:pauseAnimation(track)
end
local animationConfigs = {
    {animName = "random_1_", trackKeyX = RANDOM_X1, trackKeyY = RANDOM_Y1},
    {animName = "random_2_", trackKeyX = RANDOM_X2, trackKeyY = RANDOM_Y2},
    {animName = "random_3_", trackKeyX = RANDOM_X3, trackKeyY = RANDOM_Y3},
    {animName = "random_4_", trackKeyX = RANDOM_X4, trackKeyY = RANDOM_Y4},
    {animName = "random_5_", trackKeyX = RANDOM_X5, trackKeyY = RANDOM_Y5},
    {animName = "random_6_", trackKeyX = RANDOM_X6, trackKeyY = RANDOM_Y6},
}
...
function base_track_state.entry(this, context)
    ...
    for _, config in ipairs(animationConfigs) do
        runAndPauseAnimation(context, config.animName.."x", config.trackKeyX)
        runAndPauseAnimation(context, config.animName.."y", config.trackKeyY)
    end
end
...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        ...
        local spread_multiplier = 0.175 -- 参数
        if context:getAimingProgress() >= 1 then
            spread_multiplier = 0.1625
        end
        local y_fix_spread = 0.625
        for counter, config in ipairs(animationConfigs) do
            local seed = context:getAmmoCount()
            local x_random_spread = randFunc(seed , counter) -- 从-1到1
            context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, config.trackKeyX), (1 - (1 +x_random_spread *spread_multiplier)/2), true)
            local y_random_spread = randFunc(seed , counter + 1) -- 从-1到1
            context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, config.trackKeyY), (1 - (1 +y_random_spread *spread_multiplier *y_fix_spread)/2), true)
        end
    end
    return nil
end
