...
function bolt_caught_states.normal.update(this, context)
    ...
    local ammo = context:getAmmoCount()%7
    if context:getFireMode() ~= AUTO then
        ammo = 0
    end
    context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK),ammo,false)
end

-- 进入"不空挂"状态
function bolt_caught_states.normal.entry(this, context)
    context:runAnimation("flash",context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK),false,PLAY_ONCE_HOLD,0)
...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        ...
        elseif (context:getFireMode()==BURST) then
            local count = math.floor(context:getAmmoCount()/7)
            for i = 0, count - 1, 1 do
            context:runAnimation("shoot3", context:findIdleTrack(GUN_KICK_TRACK_LINE, false), true, PLAY_ONCE_STOP, 0)
            end
        end
    end
    return nil
