...
local function runPutAwayAnimation(context)
    ...
    if (context:getFireMode() == BURST) then
        context:runAnimation("put_away_gl", track, false, PLAY_ONCE_HOLD, put_away_time * 0.75)
    else
...
local function runReloadAnimation(context)
    ...
    if (isNoAmmo(context)) then
        if (context:getFireMode() == BURST) then
            context:runAnimation("reload_gl_empty", track, false, PLAY_ONCE_STOP, 0.2)
        else
...
local function runInspectAnimation(context)
    ...
    if (isNoAmmo(context)) then
        if (context:getFireMode() == BURST) then
            context:runAnimation("inspect_gl_empty", track, false, PLAY_ONCE_STOP, 0.2)
        else
            context:runAnimation("inspect_empty", track, false, PLAY_ONCE_STOP, 0.2)
        end
    else
        if (context:getFireMode() == BURST) then
            context:runAnimation("inspect_gl", track, false, PLAY_ONCE_STOP, 0.2)
        else
...
function base_track_state.entry(this, context)
    if (context:getFireMode() == BURST) then
        this.base_track_state.mode = 0
        context:runAnimation("static_gl_idle", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
    elseif (context:getFireMode() == AUTO) then
        this.base_track_state.mode = 2
        context:runAnimation("static_auto", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
    end
end

function base_track_state.update(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, BASE_TRACK)
    this.bolt_caught_states.cooldown = math.max(this.bolt_caught_states.cooldown - 1, 0)
    if (context:isHolding(track)) then
        if (context:getFireMode() == AUTO) then
            if (this.base_track_state.mode == 0) then
                this.bolt_caught_states.cooldown = 50
                context:stopAnimation(context:getTrack(STATIC_TRACK_LINE, INSPECT_TRACK))
                context:runAnimation("switch_auto", context:getTrack(STATIC_TRACK_LINE, SWITCH_TRACK), false, PLAY_ONCE_HOLD, 0)
                this.base_track_state.mode = 2
            else
                this.bolt_caught_states.rifle_has_ammo = context:hasBulletInBarrel()
                context:runAnimation("static_auto", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
            end
        elseif (context:getFireMode() == BURST) then
            if (this.base_track_state.mode == 2) then
                this.bolt_caught_states.cooldown = 50
                context:stopAnimation(context:getTrack(STATIC_TRACK_LINE, INSPECT_TRACK))
                context:runAnimation("switch_gl", context:getTrack(STATIC_TRACK_LINE, SWITCH_TRACK), false, PLAY_ONCE_HOLD, 0)
                this.base_track_state.mode = 0
            else
                context:runAnimation("static_gl_idle", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
            end
...
function bolt_caught_states.normal.update(this, context)
    if (not this.bolt_caught_states.rifle_has_ammo)and(this.bolt_caught_states.cooldown == 0) then
...
function bolt_caught_states.bolt_caught.update(this, context)
    if (this.bolt_caught_states.rifle_has_ammo)and(this.bolt_caught_states.cooldown == 0) then
...
function main_track_states.start.transition(this, context, input)
    if (input == INPUT_DRAW) then
        if (context:getFireMode() == BURST) then
            context:runAnimation("draw_gl", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
        else
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (context:getFireMode() == BURST) then
            context:runAnimation("shoot_gl", track, true, PLAY_ONCE_STOP, 0)
        else
