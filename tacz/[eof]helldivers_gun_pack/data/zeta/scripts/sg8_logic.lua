...
local function getReloadTimingFromParam(param)
    ...
    if (intro_empty == nil or intro_left == nil or intro_right == nil or loop == nil or ending == nil or intro_empty_feed == nil or loop_feed == nil) then
        return nil
    end
    return intro_empty, intro_left, intro_right, loop, ending, intro_empty_feed, loop_feed
end


function M.tick_reload(api)
    local ammo_consume = 0
    ...
    local intro_empty, intro_left, intro_right, loop, ending, intro_empty_feed, loop_feed = getReloadTimingFromParam(param)
    ...
    if (reloaded_count == 0) then
        if (not cache.is_tactical) then
            if (reload_time > intro_empty_feed) then
                api:consumeAmmoFromPlayer(ammo_consume)
                api:setAmmoInBarrel(true)
