...
local RANDOM_TRACK_A = increment(static_track_top)
local RANDOM_TRACK_B = increment(static_track_top)
...
function bolt_caught_states.normal.update(this, context)
    ...
    local a = math.random()
    local b = math.random()
    context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, RANDOM_TRACK_A),a,true)
    context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, RANDOM_TRACK_B),b,true)
end

-- 进入"不空挂"状态
function bolt_caught_states.normal.entry(this, context)
    context:runAnimation("random_layer_1",context:getTrack(STATIC_TRACK_LINE, RANDOM_TRACK_A),true,LOOP,0)
    context:runAnimation("random_layer_2",context:getTrack(STATIC_TRACK_LINE, RANDOM_TRACK_B),true,LOOP,0)
    context:pauseAnimation(context:getTrack(STATIC_TRACK_LINE, RANDOM_TRACK_A))
    context:pauseAnimation(context:getTrack(STATIC_TRACK_LINE, RANDOM_TRACK_B))
...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        local counter = this.gun_kick_state.melee_counter
        local animationName = "shoot_" .. tostring(counter + 1)
        this.gun_kick_state.melee_counter = (counter + 1) % 2
...
local M = {
    ...
    RANDOM_TRACK_A = RANDOM_TRACK_A,
    RANDOM_TRACK_B = RANDOM_TRACK_B,
    -- 混合轨道
