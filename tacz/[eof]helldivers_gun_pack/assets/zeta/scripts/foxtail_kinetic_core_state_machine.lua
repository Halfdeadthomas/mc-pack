...
-- 混合轨道行 的 轨道 栈顶指针
local offset_track_top = {value = 0}
-- 偏移轨道行 的 轨道 栈顶指针
...
local OFFSET_TRACK_LINE = increment(track_line_top)
local ATTACHMENT_GRIP_TRACK = increment(offset_track_top)
local GRIP_DYNAMIC_TRACK = increment(offset_track_top)
...
function fire_mode_state.draw.transition(this, context,input)
    if (input == this.INPUT_MODE_DRAW) then
        if (context:getFireMode() == SEMI) then
            ...
            return this.fire_mode_state.semi
        elseif (context:getFireMode() == BURST) then
            ...
            return this.fire_mode_state.burst
        elseif (context:getFireMode() == AUTO) then
            ...
            return this.fire_mode_state.auto
        end
    end
end
...
local function getGripType(id,context)
    if id==nil then
        return nil
    end
    local typeName = nil
    local namespace = string.sub(id,1,string.find(id,":",1)-1)
    local recognition = safeRequire(namespace.."_foxtail_attachment_type_sheet")
    if recognition ~= nil then
        for type_key,grip in pairs(recognition.GRIP) do --在识别表的GRIP字典中遍历 type_key为类型的键 grip为每种类型
            for name in pairs(grip) do -- 在每个遍历到的握把类型中遍历名称 如果出现与输入的id相同的名称停止
                if id == grip[name] then
                    typeName = type_key
                    break
                end
            end
    ...
    if typeName == nil then
        if (string.find(id, ":grip_vertical_")~=nil)then -- 如果仍不满足任何一个条件，就进行基于名称的初级判断
            typeName = "VG"-- 垂直握把
        elseif (string.find(id, ":grip_angled_")~=nil) then
            typeName = "AFG"-- 三角直握把
        end
    end
    return typeName
...
local function runDrawAnimation(grip,context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    local track_offset = context:getTrack(OFFSET_TRACK_LINE, ATTACHMENT_GRIP_TRACK)
    local gripType = getGripType(grip, context)
    if gripType == nil then
        gripType = "NOTHING"
    end
    context:runAnimation("draw", track, false, PLAY_ONCE_STOP, 0)
    context:runAnimation(gripType.."_draw", track_offset, true, PLAY_ONCE_STOP, 0)
end

-- 播放丢枪动画的方法
local function runPutAwayAnimation(grip,context)
    ...
    local offsetName = gripType.."_offset_"
    context:runAnimation("put_away", track, false, PLAY_ONCE_HOLD, put_away_time * 0.75)
    context:runAnimation(offsetName.."put_away", track_offset, true, PLAY_ONCE_HOLD, put_away_time * 0.75)
    if context:isOverHeat() then
        context:runAnimation("put_away_out_of_mag", track, false, PLAY_ONCE_HOLD, put_away_time * 0.75)
        context:runAnimation(offsetName.."put_away_out_of_mag", track_offset, true, PLAY_ONCE_HOLD, put_away_time * 0.75)
    end
    -- 设定动画进度为最后一帧
...
-- 播放换弹动画的方法
local function runReloadAnimation(grip ,context)
    context:stopAnimation(context:getTrack(OFFSET_TRACK_LINE,ATTACHMENT_GRIP_TRACK))
    -- 此处获取的轨道是位于主轨道行上的主轨道
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    local track_offset = context:getTrack(OFFSET_TRACK_LINE, ATTACHMENT_GRIP_TRACK)
    local animationName = "reload" -- 接下来是字符串处理
    local offsetName = "_offset_reload"
    local param = context:getStateMachineParams()
    local ban_tactical_reload = param.ban_tactical_reload
    local closed_bolt = param.closed_bolt
    local different_reload_time = param.different_reload_time
    local gripType = getGripType(grip, context)
    if (gripType ~= nil) then
        offsetName = gripType .. "_" .. offsetName
    else
        offsetName = "NOTHING_" .. offsetName
    end
    if context:isOverHeat() then -- 过热时开始换弹意为本次换弹为接续换弹
        if (isNoAmmo(context)) then
            animationName = animationName .. "_follow"
            offsetName = offsetName .. "_follow"
        else
            animationName = animationName .. "_continue"
            offsetName = offsetName .. "_continue"
        end
    else
        if (isNoAmmo(context)) then
            animationName = animationName .. "_empty"
            offsetName = offsetName .. "_empty"
        else
            animationName = animationName .. "_tactical"
            offsetName = offsetName .. "_tactical"
        end
    end
    if different_reload_time == 1 then
        local magLevel = context:getMagExtentLevel() -- 如果采用不同等级不同时长的换弹动画，则再追加对应尾缀
        if magLevel > 0 then
            animationName = animationName .. "_xmag_" .. tostring(magLevel)
            offsetName = offsetName .. "_xmag_" .. tostring(magLevel)
        end
    end
    -- 判断是否禁用战术换弹动画
    local hasBulletInBarrel = context:hasBulletInBarrel()
    -- 如果是open_bolt就只看弹药容量是否不为0
    if not(closed_bolt == 1) then
        hasBulletInBarrel = (context:getAmmoCount()>0)
    end
    if (ban_tactical_reload == 1) and hasBulletInBarrel then -- 禁用战术换弹为真且枪管里有子弹时，那就不播放动画了
        -- 滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木滚木
    else
        context:runAnimation(animationName, track, false, PLAY_ONCE_STOP, 0.2)
        context:runAnimation(offsetName, track_offset, true, PLAY_ONCE_STOP, 0)
    end
end

-- 播放检视动画的方法
local function runInspectAnimation(grip,context)
    -- 此处获取的轨道是位于主轨道行上的主轨道
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    local track_offset = context:getTrack(OFFSET_TRACK_LINE, ATTACHMENT_GRIP_TRACK)
    local gripType = getGripType(grip, context)
    ...
    local animationName = "inspect"
    local offsetName = gripType.."_offset_inspect"
    -- 根据当前整枪内是否还有弹药决定是播放普通检视还是空枪检视
    if (context:isOverHeat()) then
        animationName = animationName .. "_out_of_mag"
        offsetName = offsetName .. "_out_of_mag"
    end
    if (isNoAmmo(context)) then
        animationName = animationName .. "_empty"
        offsetName = offsetName .. "_empty"
...
local base_track_state = {
    grip_installed = false, grip_last = nil
}
...
function base_track_state.entry(this, context)
    ...
    if (context:getAttachment("GRIP")~="tacz:empty") then
        this.base_track_state.grip_installed = true
    end
end

function base_track_state.update(this, context)
    if base_track_state.grip_last ~= (context:getAttachment("GRIP")) then -- 检测到配件更换
        context:stopAnimation(context:getTrack(OFFSET_TRACK_LINE, ATTACHMENT_GRIP_TRACK))
        this.base_track_state.grip_last = (context:getAttachment("GRIP"))
        this.base_track_state.grip_installed = true
    end
    if context:isStopped(context:getTrack(OFFSET_TRACK_LINE, ATTACHMENT_GRIP_TRACK)) then -- 检测到有轨道停止时检查并更新一次
        this.base_track_state.grip_installed = true
    end
    if (this.base_track_state.grip_installed)then
        local track = context:getTrack(OFFSET_TRACK_LINE, ATTACHMENT_GRIP_TRACK)
        local grip = this.base_track_state.grip_last
        if (getGripType(grip, context)~=nil) then
            context:runAnimation(getGripType(grip, context).."_offset_idle", track, true, LOOP, 0)
        end
        this.base_track_state.grip_installed = false
    end
end
...
function main_track_states.start.transition(this, context, input)
    ...
    if (input == INPUT_DRAW) then
        local grip = (context:getAttachment("GRIP"))
        -- 收到 draw 信号后在主轨道行的主轨道上播放掏枪动画,然后转到闲置态
        runDrawAnimation(grip,context)
...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_PUT_AWAY) then
        local grip = this.base_track_state.grip_last
        runPutAwayAnimation(grip,context)
        this.main_track_states.final.isfinal = 1
        -- 丢枪后转到最终态
    ...
    if (input == INPUT_INSPECT and context:getAimingProgress() < 1) then
        local grip = this.base_track_state.grip_last
        runInspectAnimation(grip,context)
...
function main_track_states.reload.entry(this, context)
    local grip = this.base_track_state.grip_last
    runReloadAnimation(grip,context)
    this.main_track_states.reload.isreloading = 1
end
...
function main_track_states.reload.exit(this, context)
...
function main_track_states.inspect.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT or context:getAimingProgress() > 0) or (input == this.INPUT_MODE_BURST or input == this.INPUT_MODE_AUTO or input == this.INPUT_MODE_SEMI) then
        context:stopAnimation(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK))
        return this.main_track_states.idle
    end
...
function over_heat_states.normal.update(this, context)
    if (isOverHeat(context)) and (not(this.main_track_states.reload.isreloading >= 0)) then
        context:trigger(this.INPUT_OVER_HEAT) -- 过热状态下 且不在播放换弹动画时 进入过热状态
    end
end
...
function over_heat_states.over_heat.entry(this, context)
    -- 过热被我爆改了，现在这个动画代表正准备进行接续换弹的武器播放的循环动画
    context:runAnimation("out_of_mag", context:getTrack(BLENDING_TRACK_LINE, OVER_HEATING_TRACK), true, LOOP, 0)
end

-- 更新"过热"状态
function over_heat_states.over_heat.update(this, context)
    if (not isOverHeat(context)) or(this.main_track_states.reload.isreloading >= 0) then
        context:trigger(this.INPUT_COOLING_HEAT) -- 非过热状态下 或者 在播放换弹动画时 退出过热状态
    end
end
...
local M = {
    ...
    -- 偏移轨道
    offset_track_top = offset_track_top,
    ATTACHMENT_GRIP_TRACK = ATTACHMENT_GRIP_TRACK,
    GRIP_DYNAMIC_TRACK = GRIP_DYNAMIC_TRACK,
    -- 混合轨道
...
function M:initialize(context)
    ...
    context:ensureTracksAmount(OFFSET_TRACK_LINE, offset_track_top.value)
