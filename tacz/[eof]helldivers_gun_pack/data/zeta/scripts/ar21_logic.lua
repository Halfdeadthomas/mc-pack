...
local function transformAmmoDeploy(api) --在切换形态时重新分配弹药量和热量
    ...
    if (api:getFireMode()==BURST) then --同上但是进入下挂后所需转化比值不同
...
function M.tick_heat(api, heatTimestamp)
    ...
    if (cache == nil) then
        cache = {
            ...
            max_ammo = 0,
            max_shotgun_ammo = 0,
            mode_is_burst = false, -- 是否为下挂 默认掏出时为false
            init = false --初始化
        }
    end
    if (cache.init)then --仅在已初始化后执行判断避免在SEMI状态下掏出时才执行
        if not (cache.mode_is_burst==(api:getFireMode()==BURST))then --检测到两帧之间模式发生变化，触发弹药分配
            transformAmmoDeploy(api)
        end
    else
        cache.init = true
    end
    cache.mode_is_burst = (api:getFireMode()==BURST)
...
local function finishReload(api, is_tactical) ----已被爆改
    ...
    if (api:getFireMode() == BURST) then
        needAmmoCount = shotgun_max_ammo - api:getAmmoAmount() + (api:hasAmmoInBarrel() and 0 or 1) -- 如果是单发模式（下挂）则用下挂所需装入的弹药数量填进弹匣
    end
    if (api:isReloadingNeedConsumeAmmo()) then
        -- 需要消耗弹药（生存或冒险）的话就消耗换弹所需的弹药并将消耗的数量装填进弹匣
        api:putAmmoInMagazine(api:consumeAmmoFromPlayer(1)*needAmmoCount)
    else
...
function M.tick_reload(api)
    ...
    if api:getFireMode() == BURST then
        reload_feed, reload_cooldown, empty_feed, empty_cooldown = 1530, 2100, 1530, 2100
    end
...
function M.modify_property(api, id, original)
    ...
    if (id == "explosion_destroys_block") then
        if mode == BURST then
            return false
        end
